...
def main() -> None:
    """
    Run AdaptiveRAG Stage 4 LangGraph question-answering loop.
    ...
    user question
        ...
        -> reranker node
        -> grader node
        -> generator node OR web_search node -> generator node
        -> final answer
    """
    parser = argparse.ArgumentParser(
        description="Run AdaptiveRAG Stage 4 LangGraph CRAG pipeline."
    )
    parser.add_argument(
        "--debug",
        action="store_true",
    ...
    print("AdaptiveRAG Stage 4 is ready.")
    print("Flow: rewriter -> retriever -> reranker -> grader -> generator/web_search")
